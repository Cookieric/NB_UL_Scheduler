...
typedef struct _UE_TEMPLATE_NB
{
	...
	uint8_t	Priority;// not used for now, maybe provide NB QoS scheudling(emergency/normal devices)
	int UL_Buffer_Size;
...
// uint32_t check_if_DL_subframe(uint32_t,uint32_t,uint32_t,MIB_NB &,SIB1_NB &,uint32_t *,uint32_t);
uint32_t check_if_DL_subframe(uint32_t,uint32_t,uint32_t,MIB_NB &,SIB1_NB &);
uint32_t fill_DL_subframe_bitmap(uint32_t,uint32_t,uint32_t,MIB_NB &,SIB1_NB &,uint32_t *,uint32_t);
